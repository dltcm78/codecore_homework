...
const rl = readline.createInterface({ // creates interface
...
const toDoMenu = (options) => {
    ...
    if (caseIgnoreOptions === "v") {
        ...
        if (myTodos.length === 0) {
            ...
            for (let i = 0; i < myTodos.length; i++) { // for all element in myTodos.json array(loaded as myTodos)
                if (myTodos[i].completed === true) { // if completed: true
    ...
    } else if (caseIgnoreOptions.length > 1 && caseIgnoreOptions[0] === "c" && caseIgnoreOptions.slice(1) == parseInt(caseIgnoreOptions.slice(1))) {
        const X = parseInt(caseIgnoreOptions.slice(1));
        if (X <= myTodos.length && myTodos[X].completed === false) { // if incompleted
            myTodos[X].completed = true; // change to completed
            console.log(`\nCompleted "${myTodos[X].title}"`);
        } else if (X < myTodos.length && myTodos[X].completed === true) { // if completed
            myTodos[X].completed = false; // change to incompleted
            console.log(`\nIncompleted "${myTodos[X].title}"`);
        } else { // if X is not in the index of myTodos
    ...
    } else if (caseIgnoreOptions.length > 1 && caseIgnoreOptions[0] === "d" && caseIgnoreOptions.slice(1) == parseInt(caseIgnoreOptions.slice(1))) {
        const X = parseInt(caseIgnoreOptions.slice(1));
        ...
        if (path === "myTodos.json") { // default setting
            rl.question("\nWhere?\n>", () => {
